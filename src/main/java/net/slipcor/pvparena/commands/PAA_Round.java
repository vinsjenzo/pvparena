...
public class PAA_Round extends AbstractArenaCommand {
    ...
    public void commit(final Arena arena, final CommandSender sender, final String[] args) {
        ...
        try {
            ...
            if (args.length > 1) {
                goal = PVPArena.getInstance().getAgm().getGoalByName(args[1].toLowerCase());
            } else {
            ...
            if (goal == null) {
                arena.msg(sender, Language.parse(arena, MSG.ERROR_GOAL_NOTFOUND, args[1], StringParser.joinSet(PVPArena.getInstance().getAgm().getAllGoalNames(), " ")));
