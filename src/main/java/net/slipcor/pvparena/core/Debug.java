...
public class Debug {
    ...
    private static Logger getGlobalLogger() {
        if (logger == null) {
            ...
            try {
                ...
                PVPArena.getInstance().getLogger().log(Level.SEVERE, null, ex);
            }
        }
    ...
    private Logger getArenaLogger() {
        if (this.arenaLogger == null) {
            ...
            for (final Handler handler : this.arenaLogger.getHandlers()) {
            ...
            try {
                ...
                final File debugMainFolder = new File(PVPArena.getInstance().getDataFolder(), "debug");
                debugMainFolder.mkdirs();

                final File debugFolder = new File(debugMainFolder, this.arena.getName());
                debugFolder.mkdirs();
                ...
                this.arenaLogger.addHandler(handler);
    ...
    private boolean debugs() {
        return override || this.active || check.contains(this.debugID) || check.contains(666);
    }

    private boolean debugs(final String term) {
        return override || this.active || strings.contains(term) || check.contains(666);
    }
    ...
    public void i(final String string) {
        ...
        if (this.arena == null) {
            ...
            this.getArenaLogger().info(prefix + System.currentTimeMillis() % 1000 + ' ' + string);
        }
        if (server_log) {
    ...
    public void i(final String string, final CommandSender sender) {
        if (this.arena == null && sender == null) {
            this.i(string, "null");
            return;
        }
        if (sender == null) {
            this.arena.getDebugger().i(string);
            return;
        }
        if (!this.debugs(sender.getName())) {
            return;
        }
        if (this.arena == null && sender instanceof Player) {
        ...
        if (this.arena == null) {
            ...
            this.getArenaLogger().info(prefix + "[p:" + sender.getName() + ']' + System.currentTimeMillis() % 1000 + ' ' + string);
        }
        if (server_log) {
            System.out.print(prefix + " [" + this.arena + "] " + "[p:" + sender.getName() + ']' + System.currentTimeMillis() % 1000 + ' ' + string);
        }
    }

    public void i(final String string, final String filter) {
        if (!this.debugs(filter)) {
            return;
        }
    ...
    public static void load(final PVPArena instance, final CommandSender sender) {
        ...
        if ("none".equals(debugs)) {
            if (isPlayer) {
                ...
                PVPArena.getInstance().getLogger().info("debugging: off");
            }
            ...
            if ("all".equalsIgnoreCase(debugs) || "full".equalsIgnoreCase(debugs)) {
                ...
                if (isPlayer) {
                    ...
                    PVPArena.getInstance().getLogger().info("debugging EVERYTHING");
                }
            } else {
                ...
                if (isPlayer) {
                    ...
                    PVPArena.getInstance().getLogger().info("debugging: " + debugs);
                }
